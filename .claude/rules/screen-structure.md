# 画面のフォルダ構成ルール (Next.js App Router)

Next.js (App Router) における各画面（ルート）ごとのリソース管理と責務分離を定義するための標準ルールです。

## 1. 基本フォルダ構成
各ルートディレクトリ（`app` 配下の各フォルダ）は、以下の構成を標準とします。

```text
画面フォルダ
  ┣ page.tsx      (画面のメインエントリーポイント)
  ┣ _components/  (画面専用コンポーネント)
  ┣ _hooks/       (画面専用カスタムフック)
  ┣ _actions/     (画面専用 Server Actions)
  ┣ _consts/      (画面専用の定数・型定義)
  ┣ _utils/       (画面専用の純粋関数)
  ┗ _converts/    (画面専用の表示用変換処理)
```

## 2. 各フォルダの詳細定義

### 基本原則
- フォルダには、**その画面（およびその配下のサブディレクトリ）のみで利用するリソース**を配置します。
- 複数の画面を跨いで利用するリソースは、`@/components`, `@/lib`, `@/utils` などの適切な共通ディレクトリに配置してください。
- `src` フォルダ配下のリソースをインポートする場合は、必ずエイリアス **`@/`** を使用すること。
  - よい例）`@/app/lib/auth`
  - 悪い例）`~/app/lib/auth`

### _components
- この画面専用のカスタムコンポーネントを配置します。
- **原則**: 1つのコンポーネント（1ファイル）に対し1つの責務を持たせます。
- **クライアント/サーバー**: 基本的に `"use client"` とし、UIに関わるロジックをカプセル化します。

### _hooks
- その画面専用のロジックや状態管理をカプセル化したカスタムフックを配置します。
- **目的**: `page.tsx` や大規模なコンポーネントの肥大化を防ぎ、ロジックを宣言的に記述するために活用します。
- **原則**: 複雑な `useEffect` や複数の `useState` が絡む処理、特定の業務ロジックはフックに切り出します。

### _actions
- この画面に関連する Server Actions を配置します。
- **原則**: 1つのトランザクション（まとまったサーバー処理）につき1つのファイルを割り当てます。
- **クライアント/サーバー**: 必ず `"use server"` を冒頭に記述します。

### _consts
- その画面のみで利用する以下の項目を定義します。
  - `enum` (列挙型)
  - `const` (定義値)
  - `type` / `interface` (型、インターフェース)

### _utils
- 状態を持たない純粋関数（Pure Function）を配置します。
- **要件**: クライアント(`tsx`)からもサーバー(`_actions`)からも呼び出されることを想定し、`"use client"` や `"use server"` は**付けない**でください。

### _converts
- UI表示時に利用する変換処理（Converter）を配置します。
- **原則**: 1つの変換処理につき1つのファイル。
- **例**:
  - 金額の3桁カンマ区切り変換 (`formatCurrency`)
  - 内部コード値から表示用ラベルへのマッピング (`getStatusCodeLabel`)

## 3. 命名規則
- `_` (アンダースコア) で始まるフォルダ名は、Next.js のルーティングから明示的に除外されることを示します。
- **イベントハンドラー**: ボタンやリンクなどのUI要素から直接呼び出される関数は、`handle` で始めること。
  - 例: `handleSubmit()`, `handleSort()`, `handleOpenModal()`
- **内部関数**: UIから直接呼び出されず、他の関数から補助的に呼び出される関数は、`_` (アンダースコア) で始めること。
  - 例: `_checkValue()`, `_formatDate()`, `_calculateTotal()`

## 4. サーバサイド処理の優先順位

Next.jsにおけるサーバサイドロジックの実装方針を以下に定める。

- **Server Actionsの優先**: その画面（およびその配下のサブディレクトリ）内に収まるサーバサイド処理は、Server Actionsの利用を最大限優先すること。
- **API Routes (Route Handlers) の限定的な利用**: APIの作成は、以下の「アプリ外部とのやり取り」が発生する場合に限定すること：
  - 外部サービスとの連携（Webhookの受理、SDKからの呼び出し等）
  - 一般利用者にURLを公開する必要がある場合（認証メールのリンク先、静的ファイルの動的提供等）
  - アプリの外部（モバイルアプリ等）からのリクエスト処理
